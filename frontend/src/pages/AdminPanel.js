...
const API = 'https://yonomatka.com'; // Change this to your backend API URL
...
export default function AdminPanel({ onLogout }) {
  ...
  const [newGame, setNewGame] = useState({ name: '', open_time: '', close_time: '', game_category: 'main' });
...
const [gameTab, setGameTab] = useState('main');
  ...
  const addGame = async () => {
    ...
    const res = await apiCall('/api/admin/games', 'POST', {
  name: newGame.name,
  open_time: newGame.open_time,
  close_time: newGame.close_time,
  game_category: newGame.game_category || 'main'
});
    if (res.success) { setGames(gs => [...gs, res.game || res.data]); setNewGame({ name: '', open_time: '', close_time: '', game_category: 'main' }); showToast('Game added ✅'); fetchPageData('games'); }
...
<select style={B.input} value={newGame.game_category} onChange={e => setNewGame({ ...newGame, game_category: e.target.value })}>
