...
import { MARQUEE_TEXT } from '../data/gameData';
    ...
    const fetchGames = async () => {
      try {
        ...
        const main = allGames.filter(g =>
          ...
          const closeH = g.close_time ? parseInt(g.close_time.split(':')[0]) : 23;

          // Disawar/next-day games: close_time 0-6 AM hoti hai (next day)
          const isNextDayClose = closeH < 6;
          let isClosed = false;

          if (isNextDayClose) {
            // Yeh game next-day close hoti hai - sirf uss waqt closed hogi
            isClosed = nowH < 6 && nowMins >= closeMins;
          } else {
            // Normal game
            const isLateNight = closeMins >= 22 * 60;
            const isAfterMidnight = nowMins < 2 * 60;
            isClosed = (isLateNight && isAfterMidnight) ? false : nowMins >= closeMins;
          }

          if (isClosed) return 2;
          if (hasOpen)  return 1;
          return 0; // Market open - sabse upar
        };

        const sortGames = (list, useCloseTime = false) => [...list].sort((a, b) => {
          const diff = getStatusPriority(a) - getStatusPriority(b);
          if (diff !== 0) return diff;

          // Same status mein time ke hisaab se ascending sort
          const toSortableMins = (t) => {
            if (!t) return 9999;
            const [h, m] = t.split(':').map(Number);
            const mins = h * 60 + m;
            // Late night times (0-5 AM) ko end mein daalo - yeh actually next day ke hain
            if (h < 6) return mins + 24 * 60;
            return mins;
          };

          const timeA = useCloseTime ? toSortableMins(a.close_time) : toSortableMins(a.open_time);
          const timeB = useCloseTime ? toSortableMins(b.close_time) : toSortableMins(b.open_time);
          return timeA - timeB;
        });

        const disawarFinal = disawar.length > 0 ? disawar : allGames.filter(g => g.name?.toLowerCase().includes('disawar'));

        setGames(sortGames(main, false));           // open_time se sort
        setStarlineGames(sortGames(starline, false)); // open_time se sort  
        setDisawarGames(sortGames(disawarFinal, true)); // close_time se sort (disawar)
      } catch (err) {
    ...
    >
      ...
      <div style={{ background: 'rgba(2,20,15,0.95)', padding: '6px 0', borderBottom: '1px solid rgba(0,255,213,0.18)', overflow: 'hidden' }}>
        <marquee style={{ color: '#00ffd5', fontSize: 12, fontWeight: 800, letterSpacing: '0.5px' }}>
          {settings.ticker_text || 'Welcome To World Best Yono Matka Play App - Play Win and Enjoy!'}
        </marquee>
      </div>
